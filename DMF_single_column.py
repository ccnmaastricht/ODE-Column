...
class SingleColumnDMF:
    ...
    def update_state(self, state, params, stim, dt=1e-4):
        """
        Args:
        ...
        state['N'] += (- state['N'] / params['tau_s'] + params['sigma'] * np.sqrt(
            2 / params['tau_s']) * np.random.randn(
            params['M'])) * dt
...
def get_params_single(J_local=87.8e-3, area='MT'):
    """
    Args:
    J_local (float):    local synaptic strength
    area (str):         brain area
...
def set_stimulation_single(stim, column, layer, nu, params):
    """
    Args:
...
def set_vis_single(stim, column, nu, params):
    """
    Args:
    stim (array):   external input
    column (str):   column name
...
def update_single(state, params, stim, dt=1e-4):
    """
    Args:
    state (dict):   model state (I, A, H, R, N)
        state['I']: input current
