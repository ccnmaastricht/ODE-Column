...
if __name__ == '__main__':
    ...
    ds_file          = 'pickled_ds/ds_uc_5000_sub.pkl'
    nr_samples       = 5000
    batch_size       = 32
    total_time       = 1000
    batch_time       = 200  # no longer in use
    nr_eval_points   = 5  # no longer in use
    ...
    for epoch in range(epochs):
        for batch_idx, (true_y,) in enumerate(train_loader):
            ...
            start_y = true_y[0, :, :2].to(device)                               # run with entire trajectory, no time batches
            true_y, true_mu = true_y[:, :, :2].to(device), true_y[:, :, 2].to(device)
