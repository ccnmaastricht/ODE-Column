...
class ODEFunc(nn.Module):
    '''
    Neural network that is used by the neural ODE.
    Input nn: state (x, y, mu).
    Output nn: state (x, y).
    '''
    def __init__(self):
...
def val_ODE(nn, t_, true_y):
    '''
    Validate the ODE on some test sample(s).
    '''
    with torch.no_grad():
        start_y = true_y[0, :, :2]
...
def dynamics(t, state, time_vec, mu_vec, omega):
    '''
    The state dynamics of Hopf bifurcation with input mu.
    '''
    x, y = state[0]
...
def make_hopf_ds(save=True):
    '''
    Makes a train- and test set of Hopf bifurcation trajectories.
    The trajectories have variable starting points (x, y) and variable
    sine wave inputs (mu), i.e. variable horizontal shifts.
    '''
    if os.path.exists(ds_file):
        ...
        for itr in range(total_samples):
            # Initialize true_y0 with random starting point
...
if __name__ == '__main__':
    ...
    for batch_idx, (true_y,) in enumerate(train_loader):
        ...
        if vis_training and batch_idx % test_freq == 0:
            ...
            print('Iter {:03d} | Total Loss {:.4f}'.format(ii, val_loss.item()))
