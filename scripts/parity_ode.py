...
def make_ds(nr_inputs, nr_samples, batch_size):
    ...
    train_set = all_combinations[torch.randperm(all_combinations.size(0))]
    test_set = all_combinations[torch.randperm(all_combinations.size(0))][:4]
...
def prep_stim_ode(stim_raw, time_vec, num_columns):
    ...
    for i, stim_i in enumerate(stim_raw):
        ...
        input_all_columns = torch.tile(input_column, (num_columns,))  # TODO: change this 8 to number of columns in first area!
...
def train_parity_ode(nr_inputs, nr_samples, batch_size):
    ...
    for batch_itr in range(nr_batches):
        ...
        for itr, train_stim in enumerate(train_set):
            ...
            #     plt.plot(ode_output[:, :, 208+64+i].detach().numpy())
            #     plt.show()
        ...
        # # parity_targets = parity_targets * 3.
        #
