...
def update(state, params, stim, dt=1e-4):
    ...
    state['N'] += dt * (- state['N'] / params['tau_s'] + params['sigma'] * np.sqrt(2 / params['tau_s']) * np.random.randn(params['M']))
    state['I'] += state['N']
    ...
    X = np.float16((params['a'] * (state['H'] - state['A']) - params['b']))
